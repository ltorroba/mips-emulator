...
TEST_CASE("Test Branch instructions", "[step][Branch]") {
    ...
    SECTION("bgtz") {
        ...
        SECTION("does not branch if register is less than 0") {
            vm = new Emulator(128, program, 3);
            vm->set_register(1, -1);
            vm->set_register(2, 5);
            ...
            REQUIRE(vm->step() == 0);
            REQUIRE(vm->step() == 0);
            REQUIRE(vm->get_register(1) == -1);
        }

        SECTION("branches if register is greater than 0") {
            vm = new Emulator(128, program, 3);
            vm->set_register(1, 5);
            vm->set_register(2, 8);
